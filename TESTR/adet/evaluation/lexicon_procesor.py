...
def weighted_edit_distance(word1: str, word2: str, scores: np.ndarray, ct_labels_inv):
    print("___________________in adet/evaluation/weighted_edit_distance_________________")
    m: int = len(word1)
    n: int = len(word2)
...
def _ed_delete_cost(j, i, word1, word2, scores, ct_labels_inv):
    print("___________________in adet/evaluation/_ed_delete_cost_________________")
    ## delete a[i]
    return _get_score(scores[j], word1[j], ct_labels_inv)

@njit
def _ed_insert_cost(i, j, word1, word2, scores, ct_labels_inv):
    print("___________________in adet/evaluation/_ed_insert_cost_________________")
    ## insert b[j]
    if i < len(word1) - 1:
...
def _ed_replace_cost(i, j, word1, word2, scores, ct_labels_inv):
    print("___________________in adet/evaluation/_ed_replace_cost_________________")
...
def _get_score(scores, char, ct_labels_inv):
    print("___________________in adet/evaluation/_get_score_________________")
...
class LexiconMatcher:
    def __init__(self, dataset, lexicon_type, use_lexicon, ct_labels, weighted_ed=False):
        print("___________________in adet/evaluation/(class)LexiconMatcher/init_________________")
    ...
    def find_match_word(self, rec_str, img_id=None, scores=None):
        print("___________________in adet/evaluation/(class)LexiconMatcher/find_match_word_________________")
    ...
    def _get_lexicon_path(dataset):
        print("___________________in adet/evaluation/(class)LexiconMatcher/get_lexicon_path_________________")
    ...
    def _load_lexicon(self, dataset, lexicon_type):
        print("___________________in adet/evaluation/(class)LexiconMatcher/_load_lexicon________________")
        if not self.use_lexicon:
            return
