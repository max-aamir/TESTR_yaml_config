...
def gen_crop_transform_with_instance(crop_size, image_size, instances, crop_box=True):
    ...
    Args:
        ...
        instance (dict): an annotation dict of one instance, in Detectron2's
            dataset format.
    """
    print("___________________in adet/data/gen_crop_transform_with_instance___________________")
...
def adjust_crop(x0, y0, crop_size, instances, eps=1e-3):
    modified = False
    print("___________________in adet/data/adjust_crop___________________")
    x1 = x0 + crop_size[1]
    y1 = y0 + crop_size[0]
...
class RandomCropWithInstance(RandomCrop):
    ...
    def __init__(self, crop_type, crop_size, crop_instance=True):
        ...
        print("___________________in adet/data/class RandomCropWithInstance(RandomCrop):init___________________")
        self.crop_instance = crop_instance
        self.input_args = ("image", "boxes")

    def get_transform(self, img, boxes):
        print("___________________in adet/data/class RandomCropWithInstance(RandomCrop):get_transform___________________")
